...
## Version 2.1.21~alpha2 (July 22, 2026)

### Bug Fixes

- **Fix disk-index collision moving a second template disk to this storage** (#85): `_find_free_disk_name` only queried TrueNAS for existing `vm-<vmid>-disk-*` datasets when picking the next free index for a new allocation. Once a template's first disk was moved here and renamed `vm-<vmid>-disk-N` → `base-<vmid>-disk-N` by `create_base`, that name vanished from the `vm-` namespace the query checked, so moving a second disk of the same template reused the same index and `create_base`'s rename crashed with `dataset already exists`. The query now matches both `vm-` and `base-` prefixes for the VMID. Also fixed the same gap in `list_images`' owner-VMID detection (iSCSI and NVMe-TCP), which only recognized `vm-<vmid>-...` names and silently excluded `base-<vmid>-...` volumes from any VMID-scoped listing (used by PVE core's own `find_free_diskname`/`get_next_vm_diskname`, disk-usage displays, etc.).
- **Fix cloud-init drive not regenerated on template clone** (#84): the plugin named every volume `vol-<zname>-lun<N>` / `vol-<zname>-ns<uuid>`, including cloud-init disks. PVE core's `drive_is_cloudinit()` requires the volid to end in `cloudinit` with no `vol-` prefix or metadata suffix to recognize a cloud-init drive; since our volids never matched, core treated cloud-init drives as plain CD-ROMs and copied the drive reference verbatim on clone instead of regenerating it, so the clone's user/password/SSH keys/network config were silently never applied, and it always pointed at the *template's* cloud-init volume. Cloud-init volumes are now named exactly `vm-<vmid>-cloudinit` (`alloc_image`, `list_images`), and since that form can no longer carry the embedded LUN/NVMe-UUID metadata, `path()` and `activate_volume` resolve the transport device dynamically by zvol path when no metadata is present (`_resolve_iscsi_lun`, `_resolve_nvme_uuid`).
- **Fix cloud-init disk deletion skipping local SCSI device cleanup** (#84 follow-up): `free_image` passed the (always-undef, for cloud-init volumes) parsed metadata straight through to `_free_image_iscsi` as the LUN, so its pre-teardown SCSI-device cleanup was silently skipped for every cloud-init disk delete on iSCSI. `free_image` now resolves the LUN via `_resolve_iscsi_lun` (best-effort; a failed resolve still falls back to the prior behavior rather than blocking deletion).
- **Fix `path()` losing detailed device-lookup diagnostics on failure** (#84 follow-up): the iSCSI branch's rewrite for cloud-init support dropped the bubble-up of `_device_for_lun`'s original error (active sessions, `by-path` listing) when the LUN mapping was unchanged but the local device genuinely wasn't appearing, replacing it with a generic one-line message. The original diagnostic is now preserved and re-thrown in that case for both iSCSI and NVMe-TCP.
- **Fix `path()` doubling NVMe-TCP device-lookup latency on failure** (#84 follow-up): unlike the iSCSI branch, the NVMe-TCP branch retried the full ~5s device-discovery loop unconditionally even when the re-resolved `device_uuid` was identical to the one that just failed, doubling worst-case latency and risking a duplicate subsystem reconnect that could briefly affect other VMs sharing the same NVMe-TCP connection. It now only retries when the resolved UUID actually differs, matching the iSCSI branch's staleness check.
- **Cache the NVMe-TCP namespace-by-path lookup** (#84 follow-up): `_resolve_nvme_uuid` queried `nvmet.namespace.query` uncached, so a cloud-init volume's `activate_volume` + `path()` pair (called back-to-back on every VM start/migration/backup) cost 2 live API round trips where the analogous iSCSI lookup cost at most 1 thanks to the plugin's existing extent/mapping cache. The namespace-by-path lookup is now cached the same way, cleared by the existing `_clear_cache` mutation hooks.
