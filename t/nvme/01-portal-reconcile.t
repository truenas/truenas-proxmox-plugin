...
sysfs();
{
    ...
    my $s = scfg();
    $s->{tn_subsystem_nqn} = $OURS . "-recount";   # own NQN: the backoff is keyed by NQN
    @RAN = ();
    eval { $connect->($s, repair => 1) };

    my ($summary) = grep { /configured portal\(s\) live/ } @logs;
    ok(defined $summary, "the connect ran to its summary (nothing in backoff, no die)");
...
# Portals configured by name and nothing at all for this NQN: a cold start, or
# the disconnect->connect recovery of _nvme_device_for_uuid in a fresh worker
# whose memo is empty. The hot path does not resolve names, but with no
# controller there is nothing a name could be ambiguous with, and upstream
# always handed the configured string to nvme-cli. It must connect, not die
# with zero commands issued.
sysfs();
{
    my $s = scfg();
    $s->{tn_subsystem_nqn}    = $OURS . "-byname-cold";
    $s->{tn_discovery_portal} = "nas.example.com:4420";
    $s->{tn_portals}          = "nas2.example.com:4420";
    $DNS_DOWN = 1;    # and it must not need DNS to do so
    my $err = "";
    @RAN = ();
    eval { $connect->($s); 1 } or $err = $@;
    is($err, "", "cold start by name: no error");
    my @c = grep { /^nvme connect/ } @RAN;
    is(scalar(@c), 2, "cold start by name: one connect per configured portal");
    ok((grep { /-a nas\.example\.com / } @c) && (grep { /-a nas2\.example\.com / } @c),
        "cold start by name: the configured names are handed to nvme connect");
    $DNS_DOWN = 0;
}
...
sysfs();
{
    ...
    $CONNECT_FAILS{"10.99.0.20:4420"} = 1;
    $CONNECT_FAILS{"nas.example.com:4420"} = 1;   # by name, attempted now that nothing exists, and down too
