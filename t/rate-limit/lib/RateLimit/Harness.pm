...
sub test_scfg {
    ...
    # Re-key as the plugin expects under $scfg
    my %scfg = (
        type        => $cfg->{type},
        api_host    => $cfg->{tn_api_host}     // $cfg->{api_host}     // env_require('TN_HOST'),
        api_key     => env_require('TN_API_KEY'),  # never trust storage.cfg to expose key
        api_scheme  => $cfg->{tn_api_scheme}   // 'wss',
        api_insecure=> $cfg->{tn_api_insecure} // 0,
        tn_pool     => $cfg->{tn_pool},
        target      => $cfg->{target},
...
sub _count_logins_audit {
    ...
    if ($@) {
        warn "[harness] audit.query failed: $@; falling back to journal\n";
        return _count_logins_journal($start_iso, $end_iso);
    }
