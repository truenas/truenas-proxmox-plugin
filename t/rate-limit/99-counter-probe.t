#!/usr/bin/perl
# Diagnostic: drive N known logins through the plugin via _ws_open (each
# call authenticates), count what audit.query reports during the same
# window. If counter <<< N, the audit path is the wrong instrument for
# the rate-limit story; switch to journal.
...
for (1 .. $N) {
    ...
    if ($@) {
        say_diag("_ws_open #$_ FAILED: " . substr($@, 0, 120));
    }
}
sleep 2;
my $t_end = now_iso();

is($opened, $N, "drove $N fresh logins via plugin _ws_open");
